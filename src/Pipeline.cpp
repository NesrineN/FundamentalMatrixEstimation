...
Mat GetF(const std::vector<Point2D>& img1Pts, const std::vector<Point2D>& img2Pts, const Mat& F_RANSAC, const double& f0, const int& method){
    ...
    // Mat F_kan_init = F_RANSAC_normalized;  
    F_kan_init(0,2) /= f0;
    F_kan_init(1,2) /= f0;
    F_kan_init(2,0) /= f0;
    F_kan_init(2,1) /= f0;
    ...
    F=F.t(); // transposing back to get the F that solves for x'T F x=0 
...
// Computeing the fraction of an N x N grid over the image that contains at least one match point
...
Vec RunPipelineNoiseless(Image<Color,2> I1, Image<Color,2> I2, const std::string& I1_path, const std::string& I2_path, const Mat& K1, const Mat& K2, const double& f0, const Mat& R_gt, const Vec& t_gt, const int& method, double fx, double fy, double cx, double cy,
    ...
    // 1. Getting the matches/inliers from the image pairs using SIFT + RANSAC and 8-point algorithm as a starting point
