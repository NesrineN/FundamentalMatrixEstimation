...
int main(int argc, char* argv[]){
    ...
    // in this, we want the user to run the main file and to put as arguments: skipping frame, method (1 for FNS, 2 for GN), stride, and the index of the first image if we want to run individual test
    ...
    std::cout << "Running with skipping_frame=" << skipping_frame << ", method=" << method_name << ", stride=" << stride << std::endl;
    ...
    std::ifstream file("./data/config/dataset.txt");
    std::string line;
    ...
    // if there is an argument inserted for the pair index, we take those and output the results for that only 
    ...
    // if not pair inspection mode, we loop over all the associations and run the pipeline to get the rotation and translation errors: 
