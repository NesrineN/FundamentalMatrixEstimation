...
int main(int argc, char* argv[])
{
    ...
    std::vector<Point2D> img1Pts_inliers;
    std::vector<Point2D> img2Pts_inliers;
    for(int i=0; i<matches.size(); i++){
        ...
        img1Pts_inliers.push_back(p1);
        img2Pts_inliers.push_back(p2);
    }
    ...
    Mat Eall=Mat::zeros(9,img1Pts_inliers.size());
    std::vector<Mat> Vall;

    for(int i=0; i<img1Pts_inliers.size(); i++){
        Point2D p1=img1Pts_inliers[i];
        Point2D p2=img2Pts_inliers[i];
    ...
    for(int i=0; i<img1Pts_inliers.size(); i++){
        ...
        v1(0)=img1Pts_inliers[i].x;
        v1(1)=img1Pts_inliers[i].y;
        v1(2)=f0;

        v2(0)=img2Pts_inliers[i].x;
        v2(1)=img2Pts_inliers[i].y;
        v2(2)=f0;  
